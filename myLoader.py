...
class PngLoader(Dataset):
    def __init__(self, extracted_path, lists):
    ...
    def __getitem__(self, index):
        ...
        img_name = 'rgb_' + '0'*(5 - len(str(img_idx))) + str(img_idx) + '.jpg'
        dpt_name = 'dpt_' + '0'*(5 - len(str(img_idx))) + str(img_idx) + '.png'
        img_path = os.path.join(self.path, img_name)
        dtp_path = os.path.join(self.path, dpt_name)
...
# test
def test_loader():
    path = './nyuv2/extracted'
    batch_size = 16
    dataloader = load(path, batch_size)
